...
| Token | Hex | Used for |
|---|---|---|
| `ivory-50` | `#ffffff` | Raised surfaces, cards, form panels |
| `ivory-100` | `#fbfaf7` | Page background |
| `ivory-200` | `#f2f0e9` | Subtle fills |
...
In the hero the serif line is justified edge to edge — one letter per flex
child, `justify-between` — across a full-bleed rose band that doubles as the
hero image panel, so photography can sit behind the lockup without the
